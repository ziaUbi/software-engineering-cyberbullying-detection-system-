...
class ConfigurationParameters:
    ...
    def __init__(self) -> None:
        ...
        # Uso self._handler invece di una variabile locale 'handler'
        prod_sys_conf = self._handler.read_json_file(prod_conf_path)
        if not prod_sys_conf or self._handler.validate_json(prod_sys_conf, schema_path) is False:
            raise ValueError("Invalid production system configuration")
        self.parameters: Dict[str, Any] = prod_sys_conf

        global_path = self._package_root / "configuration" / "global_netconf.json"
        net_conf = self._handler.read_json_file(global_path)
        if not net_conf:
            raise ValueError("Global netconf missing or invalid")
        self.global_netconf: Dict[str, Any] = net_conf
